...
CLAIMS = {
    ...
    "claim2": dict(
        ...
        why_finite_fails=(
            ...
            "model never varied."),
        method=(
            "The verification follows the paper's own proof, in five parts.\n\n"
            "1. **The algebraic decomposition.** Writing B_ij for the covariance "
            "contribution of observed back-door paths and eps_ij for the "
            "unobserved part, the proof reads C_biv = (B + eps)^2 and "
            "C_mult = eps^2, hence comp = sum_{i<j} B^2 + 2*B*eps. Verified to "
            "machine precision on random models.\n"
            "2. **E[B*eps] = 0** — the step that consumes unbiasedness and "
            "independence of mechanisms.\n"
            "3. **E[comp] > 0** with a Bonferroni-corrected confidence interval "
            "strictly above zero, over six coefficient laws x two noise laws x "
            "six dimensions.\n"
            "4. **The proof's explicit witness** "
            "E[B_23^2] = E[Sigma_N,11^2]Var(Gamma_21)Var(Gamma_31).\n"
            "5. **Quantifier tightness** — the theorem says n >= 3; at n = 2 the "
            "score is identically zero, so the bound is tight rather than "
            "merely conservative.\n\n"
            "Sampling stops on a fixed *relative precision* target rather than "
            "on significance, so the stopping rule is independent of the outcome "
            "being tested."),
        limitations=[
